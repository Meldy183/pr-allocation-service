...
log.Info(ctx, "PR created with reviewers", zap.String("pr_id", pr.PullRequestID), zap.Strings("reviewers", pr.AssignedReviewers))
return pr, nil
}
...
func (s *Service) ReassignReviewer(ctx context.Context, req *domain.ReassignRequest) (string, *domain.PullRequest, error) {
log := logger.FromContext(ctx)
log.Info(ctx, "reassigning reviewer", zap.String("pr_id", req.PullRequestID), zap.String("old_user_id", req.OldUserID))
pr, err := s.storage.GetPR(ctx, req.PullRequestID)
if err != nil {
return "", nil, fmt.Errorf("%s: PR not found", domain.ErrNotFound)
}
